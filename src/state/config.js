...
const {
  ...
  getTransCodecs,
  getTransBitrates,
} = require("../api/transcode");

const storageJoi = Joi.object({
  albumArtDirectory: Joi.string().default(
  ...
  syncConfigDirectory: Joi.string().default(
    path.join(__dirname, "../../save/sync")
  ),
});
...
const transcodeOptions = Joi.object({
  ...
  ffmpegDirectory: Joi.string().default(
    path.join(__dirname, "../../bin/ffmpeg")
  ),
  defaultCodec: Joi.string()
    .valid(...getTransCodecs())
    .default("opus"),
  defaultBitrate: Joi.string()
    .valid(...getTransBitrates())
    .default("96k"),
});
...
const schema = Joi.object({
  ...
  supportedAudioFiles: Joi.object()
  ...
  webAppDirectory: Joi.string().default(
    path.join(__dirname, "../../webapp/")
  ) /**Add yousician.com/lp/ */,
...
exports.asyncRandom = (numBytes) => {
  return new Promise((resolve, reject) => {
    require("crypto").randomBytes(numBytes, (err, salt) => {
      if (err) {
        return reject("Failed to generate random bytes");
      }
...
exports.setup = async (configFile) => {
  ...
  if (!program.secret) {
    winston.info(
      "Config file does not have secret.  Generating a secret and saving"
    );
